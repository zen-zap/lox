...
impl<'de> Parser<'de> {
		...
		if matches!(self.lexer.peek(), Some(Ok(Token { kind: TokenType::RIGHT_PAREN, .. }))) {
			...
			loop {
				...
				let token = self
					.lexer
					.expect_where(
						|token| matches!(token.kind, TokenType::RIGHT_PAREN | TokenType::COMMA),
	...
	pub fn parse_expression_within(
		...
		let mut lhs = match lhs {
			// atoms
			Token { kind: TokenType::STRING, origin, .. } => {
				TokenTree::Atom(Atom::String(Token::unescape(origin)))
			},

			Token { kind: TokenType::NUMBER(n), origin: _, .. } => TokenTree::Atom(Atom::Number(n)),

			Token { kind: TokenType::FALSE, .. } => TokenTree::Atom(Atom::Bool(false)),

			Token { kind: TokenType::TRUE, .. } => TokenTree::Atom(Atom::Bool(true)),

			Token { kind: TokenType::NIL, .. } => TokenTree::Atom(Atom::Nil),

			Token { kind: TokenType::SUPER, .. } => TokenTree::Atom(Atom::Super),

			Token { kind: TokenType::THIS, .. } => TokenTree::Atom(Atom::This),

			Token { kind: TokenType::IDENT, origin, .. } => TokenTree::Atom(Atom::Ident(origin)),
			...
			Token { kind: TokenType::LEFT_PAREN, origin: _, .. } => {
				...
				self.lexer
					.expect(TokenType::RIGHT_PAREN, "Unexpected end to bracketed Expression")
		...
		loop {
			...
			let op = match op.map(|res| res.as_ref().expect("handled Err above")) {
				...
				Some(Token { kind: TokenType::LEFT_PAREN, .. }) => Op::CALL,
				Some(Token { kind: TokenType::BANG, .. }) => Op::BANG,

				Some(Token { kind: TokenType::BANG_EQUAL, .. }) => Op::BANG_EQUAL,

				Some(Token { kind: TokenType::EQUAL, .. }) => Op::EQUAL,

				Some(Token { kind: TokenType::EQUAL_EQUAL, .. }) => Op::EQUAL_EQUAL,

				Some(Token { kind: TokenType::GREATER, .. }) => Op::GREATER,

				Some(Token { kind: TokenType::GREATER_EQUAL, .. }) => Op::GREATER_EQUAL,

				Some(Token { kind: TokenType::LESS, .. }) => Op::LESS,

				Some(Token { kind: TokenType::SLASH, .. }) => Op::SLASH,

				Some(Token { kind: TokenType::MINUS, .. }) => Op::MINUS,

				Some(Token { kind: TokenType::DOT, .. }) => Op::FIELD,

				Some(Token { kind: TokenType::PLUS, .. }) => Op::PLUS,

				Some(Token { kind: TokenType::STAR, .. }) => Op::STAR,

				Some(Token { kind: TokenType::OR, .. }) => Op::OR,

				Some(Token { kind: TokenType::AND, .. }) => Op::AND,

				Some(Token { kind: TokenType::LESS_EQUAL, .. }) => Op::LESS_EQUAL,
	...
	pub fn parse_statement_within(
		...
		let mut lhs = match lhs {
			...
			Token { kind: TokenType::SUPER, origin: _, .. } => TokenTree::Atom(Atom::Super),

			Token { kind: TokenType::THIS, origin: _, .. } => TokenTree::Atom(Atom::This),

			Token { kind: TokenType::LEFT_PAREN, .. } => {
				let lhs = self.parse_expression_within(0).wrap_err("in bracketed expression")?;
			...
			Token { kind: TokenType::FUN, .. } => {
				...
				if matches!(self.lexer.peek(), Some(Ok(Token { kind: TokenType::RIGHT_PAREN, .. })))
				{
					...
					loop {
						let parameter = self
							.lexer
							.expect(TokenType::IDENT, "unexpected token")
							.wrap_err_with(|| {
				...
				let block =
					self.parse_block().wrap_err_with(|| format!("in body of function {name}"))?;

				return Ok(TokenTree::Fun { name: ident, parameters, body: Box::new(block) });
			},
			...
			Token { kind: TokenType::FOR, .. } => {
				...
				let init = self.parse_expression_within(0).wrap_err(
					"in initialization of for \
                loop",
				)?;
				...
				let cond = self.parse_expression_within(0).wrap_err(
					"in loop condition of for \
                loop",
				)?;
			...
			Token { kind: TokenType::WHILE, .. } => {
				...
				let cond =
					self.parse_expression_within(0).wrap_err("in condition of while loop")?;
				self.lexer
					.expect(TokenType::RIGHT_PAREN, "missing )")
					.wrap_err("in while loop condition")?;
			...
			Token { kind: TokenType::IF, .. } => {
				...
				if matches!(self.lexer.peek(), Some(Ok(Token { kind: TokenType::ELSE, .. }))) {
					self.lexer.next();
